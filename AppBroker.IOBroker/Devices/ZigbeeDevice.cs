...
using static AppBroker.IOBroker.Data.IoBrokerHistory;
...
namespace AppBroker.IOBroker.Devices
{
    public abstract class ZigbeeDevice : Device
    {
        ...
        {
#if DEBUG
            var files = Directory.GetFiles($"F:\\Temp\\Neuer Ordner\\{date.ToString("yyyyMMdd")}\\", $"history.{AdapterWithId}*");
#else
            ...
            foreach (var file in files)
            {
                ...
                list.Add(new IoBrokerHistory(hist, file.Split('.').SkipLast(1).LastOrDefault()));
            }
            return list;
