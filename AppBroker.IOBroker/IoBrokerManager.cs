...
public class IoBrokerManager
{
    ...
    public IoBrokerManager(NLog.Logger logger, ZigbeeConfig config)
    {
        this.logger = logger;
        this.config = config;
    }
    ...
    internal string GetHistoryPath(DateTime date, string adapterWithId, string propName) 
        => Path.Combine(config.HistoryPath, $"{date:yyyyMMdd}", $"history.{adapterWithId}.{propName}.json");
}
