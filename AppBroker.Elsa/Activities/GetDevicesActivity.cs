...
[Activity(
   Category = "Smarthome",
   Description = "Gets all devices that match the given values.",
    DisplayName = "Get Devices"
)]
public class GetDevicesActivity : Activity
{
    ...
    [ActivityOutput]
    public List<GetDeviceEvent> Output { get; set; }
    ...
    private IActivityExecutionResult OnExecuteInternal(ActivityExecutionContext context)
    {
        ...
        Output = deviceFindQuery.Select(x=>new GetDeviceEvent(x)).ToList();
